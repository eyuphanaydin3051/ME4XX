...
export default function SavePreferencesModal({
  ...
  // Combine fixed selected courses with any active variation elective courses
  const allCurrentCourses = [];
  const addedCodes = new Set();

  for (const item of selectedCourses) {
    if (!item.course) continue;
    allCurrentCourses.push(item);
    addedCodes.add(item.course.code);
  }

  if (activeVariation?.addedElectives) {
    for (const item of activeVariation.addedElectives) {
      if (!item.course || addedCodes.has(item.course.code)) continue;
      allCurrentCourses.push(item);
      addedCodes.add(item.course.code);
    }
  }

  // Save current selections & preferences to LocalStorage
  const handleSaveCurrentPlan = (e) => {
    ...
    const name =
      planNameInput.trim() ||
      `Tercih Planı ${savedPlans.length + 1} (${allCurrentCourses.length} Ders)`;

    const planPayload = {
      selectedCourses: allCurrentCourses.map((item) => ({
      ...
      coursesSummary: allCurrentCourses.map((i) => i.course.codeStr).join(', '),
    };
  ...
  const handleLoadPlan = (plan) => {
    ...
    onLoadPlan(plan);
    showBanner(`"${plan.name || 'Tercihler'}" takvime yüklendi!`);
    try {
  ...
  const handleDownloadJSON = (plan) => {
    const payload = plan || {
      name: `ODTÜ ME Tercih Planı (${allCurrentCourses.length} Ders)`,
      selectedCourses: allCurrentCourses.map((item) => ({
        courseCode: item.course.code,
        courseNumber: item.course.courseNumber,
        codeStr: item.course.codeStr,
  ...
  const copyAllSISTable = () => {
    ...
    str += `--------------------------------------------------------\n`;
    allCurrentCourses.forEach(({ course, section }) => {
      const sisCode = course.code || `5690${course.courseNumber}`;
      const reg = getRegistrationInfo(course.codeStr);
      str += `${sisCode}\t${String(section?.sectionNumber || 1).padStart(2, '0')}\t${course.codeStr} (${course.name})\t${reg ? reg.type : 'İnteraktif'}\n`;
    });
    handleCopy(str, 'all');
  };
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span className="text-[11px] text-indigo-600 dark:text-indigo-300">
                  {allCurrentCourses.length} Ders • {blockedSlots.size} Saat Bloklu
                </span>
              </div>
                      ...
                      >
                        <div className="space-y-1 min-w-0 flex-1">
                          ...
                          <div className="text-[11px] text-slate-600 dark:text-slate-400 truncate">
                            {p.selectedCourses
                              ?.map((c) => `${c.codeStr || c.courseCode} (Sec ${c.sectionNumber || 1})`)
                              .join(' • ') || 'Ders yok'}
                          </div>
                        </div>
            ...
            <div className="border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-xs">
              <table className="w-full text-left text-xs border-collapse">
                ...
                <tbody className="divide-y divide-slate-200 dark:divide-slate-800/60">
                  {allCurrentCourses.map(({ course, section }, idx) => {
                    const sisCode = course.code || `5690${course.courseNumber}`;
                    const reg = getRegistrationInfo(course.codeStr);
