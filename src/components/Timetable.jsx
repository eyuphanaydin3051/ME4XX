    ...
    <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-xl overflow-hidden flex flex-col">
      ...
      <div className="px-5 py-3.5 border-b border-slate-800 flex items-center justify-between bg-slate-950/50">
        <div className="flex items-center gap-2">
          <h2 className="text-sm font-semibold text-slate-200 uppercase tracking-wider m-0">
        ...
        <div className="flex items-center gap-4 text-xs text-slate-400">
          <div className="flex items-center gap-1.5">
            <span className="w-3 h-3 rounded bg-indigo-500/30 border border-indigo-400/60 inline-block"></span>
            <span>Sabit Dersler</span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="w-3 h-3 rounded bg-amber-500/30 border border-amber-400/60 inline-block"></span>
            <span>ME4 Seçmeli</span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="w-3 h-3 rounded bg-rose-950/70 border border-rose-800/80 inline-block bg-[radial-gradient(#ef4444_1px,transparent_1px)] [background-size:6px_6px]"></span>
        ...
        <table className="w-full border-collapse min-w-[700px] select-none text-left">
          <thead>
            <tr className="bg-slate-950/80 border-b border-slate-800 text-xs font-semibold text-slate-300">
              <th className="p-3 w-28 text-center border-r border-slate-800/80">
                Saat / Slot
              </th>
              {DAYS.map((day) => (
                <th
                  key={day.id}
                  className="p-3 text-center border-r last:border-r-0 border-slate-800/80 font-medium"
                >
                  <div className="text-slate-100 font-semibold">{day.label}</div>
          ...
          <tbody className="divide-y divide-slate-800/60 text-xs">
            {TIME_SLOTS.map((slot) => (
              <tr key={slot.index} className="hover:bg-slate-800/20 transition-colors">
                {/* Time Label Column */}
                <td className="p-2.5 text-center bg-slate-950/40 border-r border-slate-800/80">
                  <div className="font-semibold text-slate-200">{slot.start}</div>
                ...
                {DAYS.map((day) => {
                  ...
                  return (
                    <td
                      ...
                      className={`p-1.5 border-r last:border-r-0 border-slate-800/60 relative h-16 align-top transition-all ${
                        isBlocked
                          ? 'bg-rose-950/40 cursor-pointer hover:bg-rose-900/50 bg-[radial-gradient(#f43f5e_1px,transparent_1px)] [background-size:8px_8px]'
                          : occupants.length === 0
                    ...
                    >
                      ...
                      {isBlocked && occupants.length === 0 && (
                        <div className="h-full w-full rounded flex flex-col items-center justify-center text-rose-400/80 gap-0.5 pointer-events-none">
                          <Lock className="w-3.5 h-3.5 text-rose-400/90" />
                      ...
                      {occupants.map((occ, idx) => {
                        ...
                        return (
                          <div
                            key={idx}
                            className={`p-1.5 rounded-lg border shadow-sm transition-transform hover:scale-[1.02] cursor-pointer group ${style.bg} ${style.border} ${style.text}`}
                      ...
                      {previews.length > 0 && occupants.length === 0 && (
                        <div className="p-1.5 rounded-lg border-2 border-dashed border-indigo-400/80 bg-indigo-500/20 text-indigo-200 animate-pulse">
                          <div className="flex items-center justify-between gap-1 leading-none">
                          ...
                          <div className="mt-1 text-[10px] text-indigo-300 flex items-center gap-0.5">
      ...
      <div className="p-3 bg-slate-950/60 border-t border-slate-800 text-xs text-slate-400 flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
